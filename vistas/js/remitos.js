...
$(".agregarIngredientePersonalizado").on("click", function() {
    $(".ingredientesPersonalizados").append(
        ...
        '<div class="col-sm-12 col-md-6 nombreIngrediente">' +
        ...
        '<div class="col-sm-6 col-md-2 ingresoPrecio">' +
        '<input type="number" class="form-control precioPersonalizado" name="precioPersonalizado" min="1" placeholder="0" required>' +

        ' <span class="small text-muted">&nbsp; Ingrese precio</span>' +
        '</div>' +
...
$(".btnCrearRemito").click(function() {
    ...
    var checkComedor = $("#check-comedor").prop("checked");
    var checkDmc = $("#check-dmc").prop("checked");
    ...
    if (idCliente != "") {
        ...
        if (checkComedor && !checkPersonalizado) {
            ...
            if (idComedor != "") {
                ...
                $(".btnGuardarRemito").attr("disabled", false);
        ...
        if (checkPersonalizado) {
            ...
            $(".listaIngredientes").each(function() {
                ...
                let precio = ($(this).children(".ingresoPrecio").children(".precioPersonalizado").val()) * cantidad;

                var elemento = [];
                elemento.push(ingrediente, cantidad, unidad, precio);
    ...
    function cargarDetalle(productosRemitoComedor, productosRemitoDmc, productosRemitoPersonalizado) {
        ...
        if (productosRemitoComedor != "") {
            ...
            costoComedor = (productosRemitoComedor[0][0]["costo"]) * cuposComedor;
        ...
        if (productosRemitoDmc != "") {
            ...
            costoDmc = (productosRemitoDmc[0][0]["costo"]) * cuposDmc;
        ...
        if (productosRemitoPersonalizado != "") {
            ...
            for (let i = 0; i < productosRemitoPersonalizado.length; i++) {
                ...
                costoPersonalizado = Number(costoPersonalizado) + Number(productosRemitoPersonalizado[i][3]);
    ...
    setTimeout(function(e) { cargarEncabezado(clienteRemito) }, 400);
})
...
$(document).ready(function() {
    $(".btnImprimirRemito").click(function() {
        ...
        console.log(costoPersonalizado);
        ...
        var precioFinal = costoPersonalizado + costoDmc + costoComedor;

        window.open("extensiones/tcpdf/remito/remito.php?dia=" + dia + "&mes=" + mes + "&ano=" + ano + "&partido=" + partido + "&municipio=" + municipio + "&organo=" + organo + "&establecimiento=" + establecimiento + "&cuit=" + cuit + "&tipo=" + tipo + "&cuposComedor=" + cuposComedor + "&cuposDmc=" + cuposDmc + "&cuerpoRemito=" + cuerpoRemito + "&precioFinal=" + precioFinal, "_blank")
...
==============================================*/
$(document).ready(function() {
    $(".btnGuardarRemito").click(function() {
        ...
        var fechaRemito = dia + '/' + mes + '/' + ano;
        var datosRemito = ("extensiones/tcpdf/remito/remito-guardado.php?dia=" + dia + "&mes=" + mes + "&ano=" + ano + "&partido=" + partido + "&municipio=" + municipio + "&organo=" + organo + "&establecimiento=" + establecimiento + "&cuit=" + cuit + "&tipo=" + tipo + "&cuposComedor=" + cuposComedor + "&cuposDmc=" + cuposDmc + "&cuerpoRemito=" + cuerpoRemito + "&precioFinal=" + precioFinal);

        var data = {};
        data.idCliente = idCliente;
        data.fecha = fechaRemito;
        data.datos = datosRemito;
        data.costo = precioFinal;
        ...
        $.ajax({
            ...
            error: function(error) {
                ...
                swal.fire({
                    type: "success",
                    title: "El remito ha sido guardado correctamente",
                    ...
                    if (result.value) {
                        $(".btnGuardarRemito").attr("disabled", true);
        ...
        $(".btnImprimirRemito").attr("disabled", false);
...
ELIMINAR REMITO
========================================= */
$(".tablaRemitosGuardados").on('click', 'button.btnEliminarRemito', function() {

    var idRemito = $(this).attr("idRemito");

    swal.fire({
        title: 'Está seguro que desea borrar el remito?',
        ...
        confirmButtonText: 'Si, borrar remito'
    }).then(function(result) {
            if (result.value) {
                window.location = 'index.php?ruta=ver-remitos&idRemito=' + idRemito;
            }
        }
